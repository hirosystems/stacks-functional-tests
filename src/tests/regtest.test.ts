...
describe('regtest-env pox-4', () => {
  ...
  test('stack-stx (before prepare-phase)', async () => {
    ...
    poxInfo = await client.getPoxInfo();
    await waitForPreparePhase(poxInfo, -2); // two before real prepare phase

    poxInfo = await client.getPoxInfo();
    expect(isInPreparePhase(poxInfo.current_burnchain_block_height as number, poxInfo)).toBeFalsy();
    expect(
      isInPreparePhase((poxInfo.current_burnchain_block_height as number) + 2, poxInfo)
    ).toBeTruthy();
    ...
    await waitForBurnBlockHeight(stackHeight + 2); // first block which has next reward set / prepare phase started
    ...
    await timeout(1000); // TODO: flakiness? should be set once we're in the prepare phase
    const set = await getStackerSet(nextCycle);
    expect(set).toBeDefined();
    expect(JSON.stringify(set)).toContain(c32addressDecode(steph.address)[1]); // stacker is in reward set
  ...
  test('stack-stx (on prepare-phase start)', async () => {
    ...
    expect(result.burn_block_height).toBe(stackHeight + 1);
    console.log('stackHeight', result.burn_block_height, stackHeight + 1);
  ...
  test('stack-stx (reward-phase), stack-increase (reward-phase)', async () => {
    ...
    expect(datas).toContainEqual(
      expect.objectContaining({
        increase_by: increaseBy.toString(),
        total_locked: (amount + increaseBy).toString(),
        start_cycle_id: nextCycle.toString(),
  ...
  test('pool: agg increase over maxAmount', async () => {
    ...
    const fullAmount = BigInt(poxInfo.min_amount_ustx) * 4n;
    const nextCycle = poxInfo.reward_cycle_id + 1;
    ...
    const { txid: poolAlice } = await pool.client.delegateStackStx({
      stacker: alice.address,
      amountMicroStx: (fullAmount * 1n) / 4n,
      poxAddress: pool.btcAddress,
      burnBlockHeight: poxInfo.current_burnchain_block_height,
      cycles: delegateStackCycles,
      privateKey: pool.key,
    ...
    signature = pool.client.signPoxSignature({
      ...
      poxAddress: pool.btcAddress,
      signerPrivateKey: signer.signerPrivateKey,
      maxAmount: (fullAmount * 1n) / 4n, // doesn't account for increase
      authId,
    });
    ...
    expect(poolIncTx.tx_status).toBe('abort_by_response');
    expect(poolIncTx.tx_result.repr).toContain('(err 38');
    ...
    signature = pool.client.signPoxSignature({
      ...
      poxAddress: pool.btcAddress,
      signerPrivateKey: signer.signerPrivateKey,
      maxAmount: (fullAmount * 2n) / 4n, // accounts for increase
      authId,
    });
    const { txid: poolInc2 } = await pool.client.stackAggregationIncrease({
      poxAddress: pool.btcAddress,
      rewardCycle: nextCycle,
      rewardIndex: commitIndex,
      signerKey: signer.signerPublicKey,
      signerSignature: signature,
      maxAmount: (fullAmount * 2n) / 4n,
      authId,
      privateKey: pool.key,
      nonce: poolNonce++,
    });
    const poolInc2Tx = await waitForTransaction(poolInc2);
    expect(poolInc2Tx.tx_status).toBe('success');
    expect(poolInc2Tx.tx_result.repr).toContain('(ok');

    // CHECK LOCKED
    expect(await alice.client.getAccountBalanceLocked()).toBe((fullAmount * 2n) / 4n);
  ...
  test('Pool delegate cannot delegate-stack-stx on behalf of a delegator that delegated to another pool', async () => {
    ...
    const alice = getAccount(ENV.PRIVATE_KEYS[0]);
    const poolA = getAccount(ENV.PRIVATE_KEYS[1]);
    const poolB = getAccount(ENV.PRIVATE_KEYS[2]);
  ...
  test('Pool can pre-approve a signature for participants', async () => {
    ...
    const alice = getAccount(ENV.PRIVATE_KEYS[0]);
    const bob = getAccount(ENV.PRIVATE_KEYS[1]);
    const pool = getAccount(ENV.PRIVATE_KEYS[2]); // signer, not actually pooling
  ...
  test('Stacker switches signers for stack-increase', async () => {
    ...
    const alice = getAccount(ENV.PRIVATE_KEYS[0]);
    const signerA = getAccount(ENV.PRIVATE_KEYS[3]);
    const signerB = getAccount(ENV.PRIVATE_KEYS[4]);
...
//     ✓ stack-stx (in reward-phase) (214993 ms)
//     ✕ stack-stx (before prepare-phase) (212341 ms)
//     ✓ stack-stx (on prepare-phase start) (173151 ms)
//     ✓ stack-stx (in prepare-phase) (168200 ms)
//     ✓ stack-stx (reward-phase), stack-extend (reward-phase) (457973 ms)
//     ✓ stack-stx (reward-phase), stack-extend (prepare-phase) (321140 ms)
//     ✕ stack-stx (reward-phase), stack-increase (reward-phase) (77181 ms)
//     ✓ stack-stx (reward-phase), stack-increase (prepare-phase) (335007 ms)
//     ✓ pool: delegate-stack, agg-increase (prepare-phase) (64761 ms)
//     ✓ pool: agg increase over maxAmount (70097 ms)
//     ✓ pool: delegate with invalid hashbyte length (222699 ms)
//     ✓ Pool delegate can only delegate-stack-stx for the next cycle (42379 ms)
//     ✓ Cannot stack if delegating (42083 ms)
//     ✓ Pool delegate cannot delegate-stack-stx if already stacking (46055 ms)
//     ✕ Pool delegate cannot delegate-stack-stx more STX than what delegator has explicitly allowed (45939 ms)
//     ✓ Pool delegate cannot delegate-stack-stx on behalf of a delegator that delegated to another pool (38606 ms)
//     ✓ Pool delegate cannot delegate-stack-stx for the current cycle (42371 ms)
//     ✓ Pool delegate cannot delegate-stack-stx to an un-delegated entity (36334 ms)
//     ✕ Pool stacker, if actively stacked, cannot revoke delegate status for the current reward cycle (48338 ms)
//     ✓ Pool can pre-approve a signature for participants (54701 ms)
//     ✓ Stacker switches signers for stack-increase (40429 ms)
//     ✓ Stacker switches signers for stack-extend (43000 ms)
//     ✓ Call readonly with weird string (40015 ms)
//     ✓ Pool stacker can delegate-stx, Pool stacker cannot submit an invalid pox-addr-version (41038 ms)
//     ✓ Pool stacker cannot delegate to two pool operators at once (42404 ms)
//     ✓ Revoke fails if stacker is not currently delegated (38524 ms)
//     ✕ Pool delegate can successfully provide a stacking lock for a pool stacker (delegate-stack-stx) (41756 ms)
//     ✓ Pool delegate cannot delegate-stack-stx to an un-delegated solo stacker (45076 ms)
//     ✓ Pool delegate cannot delegate-stack-stx for the current cycle (53483 ms)
//     ✓ Pool delegate cannot delegate-stack-stx more STX than what delegator has explicitly allowed (47335 ms)
//     ✓ Pool delegate cannot change the pox-addr provided by delegator (42008 ms)
//     ✓ Pool delegate cannot delegate-stack-stx if the delegation expires before the next cycle ends (43200 ms)
//     ✓ Pool delegate-stack-stx fails if the delegator has insufficient balance (46139 ms)
//     ✓ Pool delegate cannot delegate-stack 0 stx, Pool delegate cannot delegate-stack-stx for 0 cycles, Pool delegate cannot delegate-stack-stx for > 12 cycles (60007 ms)
//     ✓ Pool delegate cannot submit an invalid pox-addr-ver (42065 ms)
//     ✓ Pool stacker can revoke delegate status (revoke-delegate-stx) (40824 ms)
//     ✓ Pool delegate can successfully delegate-stack-extend (46788 ms)
